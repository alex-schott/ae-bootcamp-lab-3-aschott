...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    const existingTask = db.prepare('SELECT priority FROM tasks WHERE id = ?').get(req.params.id);
    if (!existingTask) return res.status(404).json({ error: 'Task not found' });
    const stmt = db.prepare('UPDATE tasks SET title = ?, description = ?, due_date = ?, priority = ? WHERE id = ?');
    const result = stmt.run(title, description || '', due_date || null, priority || existingTask.priority, req.params.id);
...
app.patch('/api/tasks/:id', (req, res) => {
  try {
    ...
    const existingTask = db.prepare('SELECT completed, priority FROM tasks WHERE id = ?').get(req.params.id);
    if (!existingTask) return res.status(404).json({ error: 'Task not found' });
    const stmt = db.prepare('UPDATE tasks SET completed = ?, priority = ? WHERE id = ?');
    const result = stmt.run(completed === undefined ? existingTask.completed : (completed ? 1 : 0), priority || existingTask.priority, req.params.id);
    if (result.changes === 0) return res.status(404).json({ error: 'Task not found' });
    const updatedTask = db.prepare('SELECT * FROM tasks WHERE id = ?').get(req.params.id);
    res.json(updatedTask);
  } catch (error) {
